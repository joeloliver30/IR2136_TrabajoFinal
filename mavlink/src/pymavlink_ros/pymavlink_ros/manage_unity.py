...
class BatteryGPSNode(Node):
    ...
    def publish_battery(self):
        remaining = self.read_battery_status()
        if remaining is not None:
            msg = UInt16()
            msg.data = int(remaining)
    ...
    def read_battery_status(self):
        ...
        while (time.time() - start_time) < 1.0:  # Intenta durante 1 segundo
    ...
    def execute_next_point(self):
        ...
        self.connection.mav.set_position_target_global_int_send(
            ...
            0b0000111111111000,  # Solo posición
            int(lat * 1e7),
            int(lon * 1e7),
            alt,
            0, 0, 0,  # Velocidades
    ...
    def wait_until_arrival(self, target_lat, target_lon, target_alt):
        while rclpy.ok() and self.is_executing:
            ...
            if msg:
                ...
                # 🔥 Primero intento leer batería real
                current_battery = self.read_battery_status()

                # 🔥 Si no hay batería real, uso el backup
                if current_battery is None:
                    current_battery = self.last_battery_value
